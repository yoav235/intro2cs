...
BASE_URL = "https://www.cs.huji.ac.il/~intro2cs1/ex6/wiki/"#sys.argv[2]
INDEX_FILE = "small_index.txt" #sys.argv[3]
OUT_FILE = "words_dict.pickle" #sys.argv[4]
...
def total_dict():
    ...
    for page in page_lst:
        ...
        for word in text_extractor(full_url).split():
            dict_total[word] = how_much_words(full_url, dict_words, word)
    with open(OUT_FILE, "wb") as f:
...
def how_much_words(pages, word_dict, word):
    html_text = text_extractor(pages)
    for word1 in html_text.split():
        word_dict[word1] = {}
        word_dict[word][pages] = html_text.count(word)
    for key in word_dict:
        for sub_key in word_dict[key]:
            if word_dict[key][sub_key] == 0:
                word_dict[key].pop(sub_key)
    return word_dict
# ----phase c----
